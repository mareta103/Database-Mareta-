...
CREATE DATABASE dbtugas10_StoreProcedure;
...
BEGIN
    SELECT nama 
    FROM pemasok
    WHERE EXISTS (SELECT * FROM barang WHERE pemasok.kode = barang.pemasok);
    
    SELECT nama 
    FROM pemasok
    WHERE NOT EXISTS (SELECT * FROM barang WHERE pemasok.kode = barang.pemasok);
END//
DELIMITER;
...
BEGIN 
    SELECT a.nama
    FROM barang a, pemasok b
    WHERE a.pemasok = b.kode AND b.nama = xpemasok;
END //
DELIMITER;

-- 4. menampilkan suatu jenis barang dan dipasok dari kota tertentu
DELIMITER //
CREATE PROCEDURE cetakJenis (IN xjenis VARCHAR(20), xkota VARCHAR(15))
BEGIN
    SELECT * 
    FROM barang, pemasok
    WHERE barang.pemasok = pemasok.kode AND barang.jenis = xjenis AND pemasok.kota = xkota;
END //
DELIMITER;

-- 5. mengetahui jumlah pemasok dari suatu kota
DELIMITER //
CREATE PROCEDURE jumlahPemasok (IN xkota VARCHAR(15))
BEGIN
    SELECT COUNT(kota)
    FROM pemasok
    WHERE EXISTS (SELECT * FROM barang WHERE pemasok.kode = barang.pemasok) AND kota = xkota;
END //
DELIMITER;

--ini cuma coba aja
SELECT COUNT(DISTINCT pemasok.nama)
    FROM pemasok, barang
    WHERE pemasok.kode = barang.pemasok AND pemasok.kota = "semarang";
